...
async function buildKpis(s) {
  ...
  if (isSales) {
    const todays = await dbApi.listSales({ date: today });
    const paid = todays.filter((x) => x.status === 'Paid');
    const salesTotal = paid.reduce((sum, x) => sum + x.total, 0);
    const open = (await dbApi.listSales({ status: 'Open' })).length;
  ...
  if (isInventory) {
    const ings = await dbApi.listIngredients();
  ...
  if (isManager) {
    const onShift = (await dbApi.listAttendance()).filter((a) => a.time_out === '').length;
  ...
  if (role === 'Cashier') {
    const mine = (await dbApi.listSales({ date: today, cashierId: s.userId })).length;
    kpis.push({ label: 'My Bills Today', value: String(mine), sub: 'bills you created' });
  } else if (role === 'Staff') {
    const pending = (await dbApi.listLeave({ userId: s.userId, status: 'Pending' })).length;
    kpis.push({ label: 'My Pending Leave', value: String(pending), sub: 'awaiting approval' });
  } else {
    // Admin / Manager
    const pending = (await dbApi.listLeave({ status: 'Pending' })).length;
...
async function route(req, res, getSession) {
  ...
  sendJson(res, 200, { kpis: await buildKpis(s) });
  return true;
}
