...
async function getCategories(req, res, getSession) {
  if (!requireRole(req, res, getSession, INVENTORY_ROLES)) return;
  sendJson(res, 200, { categories: await dbApi.listCategories() });
}

async function getIngredients(req, res, getSession, query) {
  if (!requireRole(req, res, getSession, INVENTORY_ROLES)) return;
  const supplierId = query.get('supplier') ? Number(query.get('supplier')) : undefined;
  const rows = await dbApi.listIngredients({
...
async function resolveSupplier(data) {
  if (data.supplierId) {
    const s = await dbApi.findSupplierById(Number(data.supplierId));
    return s ? s.id : null;
  }
  const name = (data.supplier || '').trim();
  if (name) return await dbApi.ensureSupplier(name);
  return null;
}
...
async function postIngredient(req, res, getSession) {
  ...
  const supplierId = await resolveSupplier(data);
  const id = await dbApi.createIngredient({
    name, unit,
    category: (data.category || '').trim(),
    supplierId,
    quantity, reorderLevel, expirationDate,
  });
  sendJson(res, 201, { ingredient: serializeIngredient(await dbApi.findIngredientById(id)) });
}

async function getIngredient(req, res, getSession, id) {
  if (!requireRole(req, res, getSession, INVENTORY_ROLES)) return;
  const row = await dbApi.findIngredientById(id);
  if (!row) return sendJson(res, 404, { message: 'Ingredient not found.' });
  sendJson(res, 200, {
    ingredient: serializeIngredient(row),
    transactions: (await dbApi.listTransactions(id)).map(serializeTxn),
  });
}
...
async function patchIngredient(req, res, getSession, id) {
  ...
  const supplierId = (data.supplierId || data.supplier) ? await resolveSupplier(data) : row.supplier_id;
  await dbApi.updateIngredientMeta(id, {
    name, unit,
    category: (data.category != null ? String(data.category) : row.category).trim(),
    supplierId, reorderLevel, expirationDate,
  });
  sendJson(res, 200, { ingredient: serializeIngredient(await dbApi.findIngredientById(id)) });
}
...
async function postTransaction(req, res, getSession, id) {
  ...
  await dbApi.applyTransaction({
    ingredientId: id, txnType: type, quantity: delta,
    reference: (data.reference || '').trim(),
    userId: s.userId, userName: await sessionName(s),
  });
  const updated = await dbApi.findIngredientById(id);
...
async function getAlerts(req, res, getSession) {
  if (!requireRole(req, res, getSession, INVENTORY_ROLES)) return;
  const items = (await dbApi.listIngredients()).map(serializeIngredient).filter((i) => i.lowStock);
  sendJson(res, 200, { alerts: items });
}

// ---- purchase orders ------------------------------------------------------

async function getPurchaseOrders(req, res, getSession, query) {
  if (!requireRole(req, res, getSession, MANAGER_ROLES)) return;
  const supplierId = query.get('supplier') ? Number(query.get('supplier')) : undefined;
  const rows = await dbApi.listPurchaseOrders({
    supplierId: Number.isFinite(supplierId) ? supplierId : undefined,
...
async function postPurchaseOrder(req, res, getSession) {
  ...
  for (const it of rawItems) {
    ...
    if (!Number.isFinite(ingredientId) || !await dbApi.findIngredientById(ingredientId)) continue;
  ...
  const id = await dbApi.createPurchaseOrder({
    supplierId, notes: (data.notes || '').trim(),
    createdBy: s.userId, createdByName: await sessionName(s), items,
  });
  sendJson(res, 201, { order: await poDetail(id) });
}

async function poDetail(id) {
  const p = await dbApi.findPurchaseOrderById(id);
  if (!p) return null;
  const items = (await dbApi.listPoItems(id)).map((it) => ({
...
async function getPurchaseOrder(req, res, getSession, id) {
  if (!requireRole(req, res, getSession, MANAGER_ROLES)) return;
...
async function patchPurchaseOrder(req, res, getSession, id) {
  ...
  if (target === 'Sent') {
    ...
    await dbApi.setPurchaseOrderStatus(id, 'Sent');
    ...
    await dbApi.setPurchaseOrderStatus(id, 'Completed');
  } else {
    return sendJson(res, 400, { message: 'Unsupported status change.' });
  }
  sendJson(res, 200, { order: await poDetail(id) });
}
...
async function receivePurchaseOrder(req, res, getSession, id) {
  ...
  for (const r of receipts) {
    const item = await dbApi.findPoItemById(Number(r.itemId));
  ...
  for (const { item, qty } of toApply) {
    await dbApi.applyTransaction({
      ...
      reference: p.po_number, poId: id, userId: s.userId, userName: await sessionName(s),
    });
    await dbApi.addPoItemReceived(item.id, qty);
  }

  // Recompute status: fully received -> Completed, otherwise Partially Received.
  const items = await dbApi.listPoItems(id);
  const allReceived = items.every((it) => it.received_qty + 1e-9 >= it.quantity);
  await dbApi.setPurchaseOrderStatus(id, allReceived ? 'Completed' : 'Partially Received');
  sendJson(res, 200, { order: await poDetail(id) });
}
...
async function route(req, res, getSession) {
  ...
  // /api/alerts
  if (pathName === '/api/alerts' && method === 'GET') { await getAlerts(req, res, getSession); return true; }
  ...
  // /api/purchase-orders ...
  if (pathName === '/api/purchase-orders' && method === 'GET') { await getPurchaseOrders(req, res, getSession, query); return true; }
  ...
  if ((m = pathName.match(/^\/api\/purchase-orders\/(\d+)$/))) {
    const id = Number(m[1]);
    if (method === 'GET') { await getPurchaseOrder(req, res, getSession, id); return true; }
