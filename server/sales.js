...
async function saleDetail(id) {
  const s = await dbApi.findSaleById(id);
  if (!s) return null;
  const items = (await dbApi.listSaleItems(id)).map((it) => ({
...
async function getSales(req, res, getSession, query) {
  ...
  const rows = await dbApi.listSales({
...
async function getSale(req, res, getSession, id) {
  if (!requireRole(req, res, getSession, SALES_ROLES)) return;
...
async function postSale(req, res, getSession) {
  ...
  const id = await dbApi.createSale({
    tableLabel: (data.tableLabel || '').trim(),
    subtotal, discount: round2(discount), tax, total,
    cashierId: s.userId, cashierName: await sessionName(s), items,
  });
  sendJson(res, 201, { sale: await saleDetail(id) });
}
...
async function settle(req, res, getSession, id) {
  ...
  await dbApi.settleSale(id, { paymentMethod, amountTendered: round2(amountTendered), changeDue });
...
async function route(req, res, getSession) {
  ...
  let m;
  if ((m = p.match(/^\/api\/sales\/(\d+)$/)) && method === 'GET') { await getSale(req, res, getSession, Number(m[1])); return true; }
