...
// ---- employees ----
async function getEmployees(req, res, getSession) {
  if (!requireRole(req, res, getSession, MANAGER_ROLES)) return;
  sendJson(res, 200, { employees: (await dbApi.listUsers()).map((u) => ({ id: u.id, name: u.full_name, role: u.role })) });
}

// ---- attendance ----
async function getMyAttendance(req, res, getSession) {
  const s = requireRole(req, res, getSession, ALL_ROLES);
  if (!s) return;
  const open = await dbApi.findOpenAttendance(s.userId);
  const logs = (await dbApi.listAttendance({ userId: s.userId })).slice(0, 30).map(serAttendance);
  sendJson(res, 200, { open: open ? serAttendance(open) : null, logs });
}

async function postTimeIn(req, res, getSession) {
  const s = requireRole(req, res, getSession, ALL_ROLES);
  if (!s) return;
  // AC: prevent a duplicate time-in without a corresponding time-out.
  if (await dbApi.findOpenAttendance(s.userId)) {
    return sendJson(res, 409, { message: 'You are already timed in. Time out before clocking in again.' });
  }
  const id = await dbApi.createTimeIn({ userId: s.userId, employeeName: await sessionName(s), workDate: todayStr(), timeIn: nowIso() });
  sendJson(res, 201, { record: serAttendance((await dbApi.listAttendance({ userId: s.userId })).find((r) => r.id === id)) });
}

async function postTimeOut(req, res, getSession) {
  const s = requireRole(req, res, getSession, ALL_ROLES);
  if (!s) return;
  const open = await dbApi.findOpenAttendance(s.userId);
  if (!open) return sendJson(res, 409, { message: 'No open time-in found. Clock in first.' });
  await dbApi.setTimeOut(open.id, nowIso());
  const updated = (await dbApi.listAttendance({ userId: s.userId })).find((r) => r.id === open.id);
  sendJson(res, 200, { record: serAttendance(updated) });
}

// Manager view of everyone's logs, searchable by employee + date.
async function getAllAttendance(req, res, getSession, query) {
  if (!requireRole(req, res, getSession, MANAGER_ROLES)) return;
  const employee = query.get('employee') ? Number(query.get('employee')) : undefined;
  const date = query.get('date') || undefined;
  const logs = (await dbApi.listAttendance({ userId: Number.isFinite(employee) ? employee : undefined, date })).map(serAttendance);
...
async function getShifts(req, res, getSession, query) {
  const s = requireRole(req, res, getSession, ALL_ROLES);
  if (!s) return;
  ...
  const date = query.get('date') || undefined;
  const shifts = (await dbApi.listShifts({ userId, date })).map(serShift);
  // AC: approved leave is reflected in the schedule.
  const leave = (await dbApi.listApprovedLeave({ userId, date })).map(serLeave);
...
async function postShift(req, res, getSession) {
  ...
  const id = await dbApi.createShift({ ...v, createdBy: s.userId });
  sendJson(res, 201, { shift: serShift(await dbApi.findShiftById(id)) });
}

async function patchShift(req, res, getSession, id) {
  const s = requireRole(req, res, getSession, MANAGER_ROLES);
  if (!s) return;
  const existing = await dbApi.findShiftById(id);
  ...
  await dbApi.updateShift(id, v);
  sendJson(res, 200, { shift: serShift(await dbApi.findShiftById(id)) });
}

async function deleteShiftHandler(req, res, getSession, id) {
  if (!requireRole(req, res, getSession, MANAGER_ROLES)) return;
  if (!await dbApi.findShiftById(id)) return sendJson(res, 404, { message: 'Shift not found.' });
  await dbApi.deleteShift(id);
  sendJson(res, 200, { ok: true });
}

// Shared validation for create/edit. Returns { error } or resolved fields.
async function validateShift(data) {
  const errors = {};
  const userId = Number(data.employeeId);
  const emp = Number.isFinite(userId) ? await dbApi.findUserById(userId) : null;
...
// ---- leave ----
async function getLeave(req, res, getSession, query) {
  const s = requireRole(req, res, getSession, ALL_ROLES);
  if (!s) return;
  const isManager = MANAGER_ROLES.includes(s.role);
  const filterEmp = query.get('employee') ? Number(query.get('employee')) : undefined;
  ...
  const requests = (await dbApi.listLeave({ userId, status })).map(serLeave);
...
async function postLeave(req, res, getSession) {
  ...
  const id = await dbApi.createLeave({ userId: s.userId, employeeName: await sessionName(s), leaveType, startDate, endDate, reason });
  sendJson(res, 201, { request: serLeave(await dbApi.findLeaveById(id)) });
}

async function patchLeave(req, res, getSession, id) {
  const s = requireRole(req, res, getSession, MANAGER_ROLES);
  if (!s) return;
  const existing = await dbApi.findLeaveById(id);
  ...
  await dbApi.setLeaveStatus(id, status, s.userId, await sessionName(s));
  sendJson(res, 200, { request: serLeave(await dbApi.findLeaveById(id)) });
}
...
async function route(req, res, getSession) {
  ...
  if (p === '/api/employees' && method === 'GET') { await getEmployees(req, res, getSession); return true; }

  if (p === '/api/attendance/me' && method === 'GET') { await getMyAttendance(req, res, getSession); return true; }
  if (p === '/api/attendance/time-in' && method === 'POST') { await postTimeIn(req, res, getSession); return true; }
  if (p === '/api/attendance/time-out' && method === 'POST') { await postTimeOut(req, res, getSession); return true; }
  if (p === '/api/attendance' && method === 'GET') { await getAllAttendance(req, res, getSession, q); return true; }

  if (p === '/api/shifts' && method === 'GET') { await getShifts(req, res, getSession, q); return true; }
  ...
  if ((m = p.match(/^\/api\/shifts\/(\d+)$/))) {
    ...
    if (method === 'DELETE') { await deleteShiftHandler(req, res, getSession, id); return true; }
  }

  if (p === '/api/leave' && method === 'GET') { await getLeave(req, res, getSession, q); return true; }
